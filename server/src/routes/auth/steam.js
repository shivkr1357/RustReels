...
function getCustomReturnURL(state) {
  return new SteamAuth({
    realm: BACKEND_URL,
    returnUrl: BACKEND_URL + '/api/auth/steam/callback?state=' + state,
  ...
  router.get("/", async (req, res, next) => {
    try {
      ...
      if (req.query.redirect) {
        redirectUrl = await getCustomReturnURL(req.query.redirect).getRedirectUrl()
      } else {
    ...
    try {
      ...
      if (dbUser) {
        ...
        let newUser = new User({
          ...
          // add default rakeBack object on steam login
          rakeBack: defaultRakeBackObject()
        });
