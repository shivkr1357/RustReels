...
router.get("/dashboard", async (req, res, next) => {
  try {
    ...
    // // Get wheel games today
    // const wheelGamesToday = await WheelGame.find({
    //   created: { $gte: today },
    // }).lean();
    // // Get wheel games yesterday
    // const wheelGamesYesterday = await WheelGame.find({
    //   created: { $gte: yesterday, $lt: today },
    // }).lean();
    // // Get wheel games from this week
    // const wheelGamesThisWeek = await WheelGame.find({
    //   created: {
    //     $gte: firstDayOfTheWeek,
    //     $lt: firstDayOfNextWeek,
    //   },
    ...
    const gamesStatistics = {
      ...
      // wheel: {
      //   totalValueToday: wheelGamesToday
      //     .map(game =>
      //       game.players.map(player => player.betAmount).reduce(sumReducer, 0)
      //     )
      //     .reduce(sumReducer, 0),
