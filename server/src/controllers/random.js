...
// Generate battles random data
const generateBattlesRandom = async (gameId, privateSeed) => {
  return new Promise(async (resolve, reject) => {
    try {

      const publicSeed = await getPublicSeed();

      // Combine privateSeed, gameId, and publicSeed to create a new hash
      const combinedSeed = `${privateSeed}-${gameId}-${publicSeed}`;
      const hash = crypto.createHash("sha256").update(combinedSeed).digest("hex");

      // Resolve promise and return data
      resolve({ hash, publicSeed });
    } catch (error) {
      reject(error);
    }
  });
...
// Generate wheel first round random data
const generateWheelFirstRandom = async () => {
  return new Promise(async (resolve, reject) => {
    try {
      // Get a new public seed from blockchain
      const publicSeed = await getPublicSeed();
...
// Generate wheel random data
const generateWheelRandom = async (
  gameId,
  privateSeed,
  publicSeed,
  roundNumber
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Generate a random, repeatable module to determine round result (index from 0-24)
      const module = chance.floating({ min: 0, max: 1, fixed: 7 });
...
// Generate wheel mystery random data
const generateWheelMysteryRandom = async (
  gameId,
  privateSeed,
  publicSeed,
  roundNumber
