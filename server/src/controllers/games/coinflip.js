      ...
      async (players, color, betAmount, private, costMultiplier) => {
        ...
        try {
          ...
          if (private) {
            ...
            addWageredToRakeBack(user.id, Math.abs(gamePrice))
            ...
            await checkAndApplyAffiliatorCut(user.id, houseEdge, betAmount);
    ...
    socket.on("join-game", async (gameId, color, recaptchaResponse) => {
      ...
      try {
        ...
        if (game.privateGame) {
          ...
          // add wagered to rakeback
          addWageredToRakeBack(user.id, Math.abs(betAmount))
          ...
          // add wagered to rakeback
          addWageredToRakeBack(user.id, Math.abs(game.betAmount))
          ...
          // Apply cut of house edge to user's affiliator
          await checkAndApplyAffiliatorCut(user.id, houseEdge. game.betAmount);
        }
