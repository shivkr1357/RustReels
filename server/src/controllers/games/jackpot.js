...
const listen = io => {
  ...
  const payoutWinnerLow = async () => {
    try {
      ...
      for (const bet of GAME_STATE.players) {
        if (bet._id === GAME_STATE.winner._id) userBet += bet.betAmount
        totalBet += bet.betAmount
      }

      const houseRake = totalBet * config.games.jackpot.feePercentage;
      const feeMultiplier = 1 - config.games.jackpot.feePercentage;
      const wonAmount = totalBet * feeMultiplier;
      ...
      await checkAndApplyAffiliatorCut(GAME_STATE.winner._id, houseRake, userBet);
  ...
  const payoutWinnerMiddle = async () => {
    try {
      ...
      for (const bet of GAME_STATE_MIDDLE.players) {
        if (bet._id === GAME_STATE_MIDDLE.winner._id) userBet += bet.betAmount
        totalBet += bet.betAmount
      }
      ...
      // Apply cut of house edge to user's affiliator
      await checkAndApplyAffiliatorCut(GAME_STATE_MIDDLE.winner._id, houseRake, userBet);
  ...
  const payoutWinnerHigh = async () => {
    try {
      ...
      for (const bet of GAME_STATE_HIGH.players) {
        if (bet._id === GAME_STATE_HIGH.winner._id) userBet += bet.betAmount
        totalBet += bet.betAmount
      }
      ...
      // Apply cut of house edge to user's affiliator
      await checkAndApplyAffiliatorCut(GAME_STATE_HIGH.winner._id, houseRake, userBet);
