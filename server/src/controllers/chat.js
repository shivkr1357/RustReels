...
const RAIN_STATE = {
  ...
  startTime: null,
  endTime: null,
  lastAutomatedRain: null, // Timestamp of last automated rain
};
const TRIVIA_STATE = {
...
// Create a new automated rain
const createAutomatedRain = async (amount) => {
  try {
    // Validate parameters
    ...
    // Rate limiting check
    const now = Date.now();
    if (RAIN_STATE.lastAutomatedRain && (now - RAIN_STATE.lastAutomatedRain) < 30000) { // 30 seconds minimum between automated rains
      throw new Error('Rain creation too frequent');
    }

    // If there's an active rain, just add to it via tip
    if (RAIN_STATE.active) {
      // Validate new total amount
      const newTotal = RAIN_STATE.prize + amount;
      if (newTotal > 1000) { // Maximum rain amount
        throw new Error('Rain amount would exceed maximum');
      }

      RAIN_STATE.prize = newTotal;
      if (ioInstance) {
        ioInstance.of("/chat").emit("update-rain", {
          prize: RAIN_STATE.prize
        });
      }
      return true;
    }

    // If no active rain, create one with .create-rain command
    const rainCommand = {
      content: `.create-rain ${amount} 120`,
      user: {
        id: 'SYSTEM_AUTOMATED_RAIN',
        wallet: amount, // Only allow exact amount needed
        rank: 5 // Admin rank to ensure command works
      }
    };

    // Update last rain timestamp
    RAIN_STATE.lastAutomatedRain = now;

    // Process the rain creation command
    await processRainCommand(rainCommand);
    return true;

  } catch (error) {
    console.error('[Rain] Create automated rain error:', error);
    return false;
  }
};

// Helper function to process rain commands
const processRainCommand = async (command) => {
  try {
    // Validate command source
    if (!command.user || !command.user.id) {
    ...
    // Create the rain
    RAIN_STATE.prize = amount;
    RAIN_STATE.timeLeft = duration * 1000;
    RAIN_STATE.automated = command.user.id === 'SYSTEM_AUTOMATED_RAIN';
    RAIN_STATE.startTime = Date.now();
    RAIN_STATE.endTime = Date.now() + (duration * 1000);
    RAIN_STATE.active = true;
    RAIN_STATE.players = [];

    if (ioInstance) {
      ioInstance.of("/chat").emit("new-rain", RAIN_STATE);
    }

    // Set timeout to end the rain
    setTimeout(() => {
      if (RAIN_STATE.active) {
        endCurrentRain();
      }
    ...
    console.error('[Rain] Process rain command error:', error);
    throw error; // Propagate error to caller
  }
};

const startNewRain = async (amount) => {
  try {
    RAIN_STATE.prize = amount;
    RAIN_STATE.active = true;
  } catch (error) {
    console.error('[Rain] Start rain error:', error);
  }
};
...
const endCurrentRain = async () => {
  try {
    ...
    // Calculate winnings
    const playerCount = RAIN_STATE.players.length;
    if (playerCount > 0) {
      const winPerPlayer = RAIN_STATE.prize / playerCount;
      
      // Add balance to each player
      for (const userId of RAIN_STATE.players) {
        await User.updateOne(
          { _id: userId },
          { $inc: { wallet: winPerPlayer } }
        );
      }
    }
    ...
    RAIN_STATE.prize = 0;
    RAIN_STATE.automated = false;
    RAIN_STATE.startTime = null;
    RAIN_STATE.endTime = null;

    // Notify clients
  } catch (error) {
    console.error('[Rain] End rain error:', error);
  }
};
  ...
  io.of("/chat").on("connection", socket => {
      ...
      try {
        ...
        if (command.includes('/selfexclude')) {
          ...
          if (args[1] === 'All_Modes') {
            ...
            dbUser.selfExcludes.wheel = Math.max(dbUser.selfExcludes.wheel, Date.now() + args[2]);
        ...
        if (command.includes("/tip")) {
          ...
          if (timp_ramas > 0)
            return socket.emit(
              "notify-error",
              "You are banned from crusade.gg, you can't use the tip feature!"
            )
          ...
          // Check wager requirement for users that aren't a sponsor or higher
          if (dbUser.wagerNeededForWithdraw > 0 && dbUser.rank < 2)
            return socket.emit(
              'notify-error',
              `You need to wager another $${Math.floor(dbUser.wagerNeededForWithdraw * 100) / 100} before you can tip other players!`
            )
          // TIPPING
    ...
    socket.on("enter-rain", async recaptchaResponse => {
      try {
        ...
        const dbUser = await User.findOne({ _id: user.id });
        if (!dbUser) {
          return socket.emit("rain-join-error", "User not found!");
        }
    ...
    socket.on("disconnect", () => {
      //Update online users count
      io.of("/chat").emit("users-online", Object.keys(io.of("/chat").sockets).length);
    });

    // Create a new automated rain
    socket.on("create-automated-rain", async (amount, duration) => {
      try {
