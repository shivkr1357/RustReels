...
export const RECAPTCHA_SITE_KEY = "6LdXe4QqAAAAAB7nlFCHvyniuLOFEtJDFu6zcqBz"; //localhost do 6LcWZY8iAAAAAAFUMhm5TG1uNWLRj5lOP2bgT_VE //clashpot do 6Ldlc3UmAAAAAKSCIme-cMh6_TO2stmnxOaSK6Dk
...
// Claim rakeback bonus from profile
export const claimRakeBackBonus = async (type) => {
  return new Promise(async (resolve, reject) => {
    try {
      const response = await API.post("/user/rakeback/claim", {
        type
      });
      resolve(response.data)
    } catch (error) {
...
export const getRakeBackBonus = async () => {
  return new Promise(async (resolve, reject) => {
    try {
      const response = await API.get('/user/rakeback')
      resolve(response.data)
    } catch (error) {
      reject(error)
    }
...
// Get current wheel game and history
export const getWheelSchema = async () => {
  return new Promise(async (resolve, reject) => {
    try {
...
// Get current kings game and history
export const getKingsSchema = async () => {
  return new Promise(async (resolve, reject) => {
    try {
      const response = await API.get("/kings");
      resolve(response.data);
    } catch (error) {
      reject(error);
    }
