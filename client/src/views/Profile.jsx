...
import { claimRakeBackBonus, getRakeBackBonus, getUserProfileData, getUserVipData } from "../services/api.service";
...
// Assets
import stackGold from "../assets/rakeback/stack-gold.png";
...
const Profile = ({ isLoading, isAuthenticated, user, logout }) => {
  ...
  const [rakeBack, setRakeBack] = useState(samepleRakeBackObject);
  ...
  // RAKEBACK BONUS SYSTEM
  const claimBonus = async (type) => {
    try {
      const result = await claimRakeBackBonus(type)
      addToast(`Successfully collected $${result.rakeBackValue} from ${result.type} rakeback!`, { appearance: 'success' })

      // request the new updated rakeback
      const rakeback = await getRakeBackBonus()
      updateRakeBack(rakeback)
    } catch(e) {
      addToast(e?.response?.data?.error ? e.response.data.error : `There was an error claiming the bonus, try`, { appearance: "error" });
    }
  }

  const updateRakeBack = (rb) => {
    setRakeBack((prevData) => ({
      ...prevData,
      ...rb
    }));
  }
        ...
        <Container maxWidth="lg">
          <Grow in timeout={620}>
            <Box className={classes.profile}>
              ...
              <Box className={classes.userWrap} style={{ backgroundColor: "#1b2129", }}>
                <Box className={classes.user}>
                  ...
                  <Box position="relative" className={classes.progressbox} style={{ maxWidth: "300px", }}>
                    <TextField
                      ...
                      value={!isLoading && !isAuthenticated ? "" : displayname}
                    />
                    <Button
              ...
              <div className={classes.rb}>Rakeback</div>

              {/* RAKEBACK SYSTEM */}
              <Grid className={classes.rbgrid} container>
                <Box>
                  <img src={stackGold} alt="Stack gold"/>
                  <div>Daily Bonus</div>
                  {rakeBack.daily.amount ? (<div>Current bonus: <span className={classes.rbprice}>${ parseFloat(rakeBack.daily.amount).toFixed(2) }</span></div>) : <div></div>}
                  <div onClick={() => { claimBonus('daily') }}>{!isNaN(parseInt(rakeBack.daily.amount)) ? 'Claim Bonus' : rakeBack.daily.ftime}</div>
                </Box>
                <Box>
                  <img src={stackGold} alt="Stack gold"/>
                  <div>Weekly Bonus</div>
                  {rakeBack.weekly.amount ? (<div>Current bonus: <span className={classes.rbprice}>${ parseFloat(rakeBack.weekly.amount).toFixed(2) }</span></div>) : <div></div>}
                  <div onClick={() => { claimBonus('weekly') }}>{!isNaN(parseInt(rakeBack.weekly.amount)) ? 'Claim Bonus' : rakeBack.weekly.ftime}</div>
                </Box>
                <Box>
                  <img src={stackGold} alt="Stack gold"/>
                  <div>Monthly Bonus</div>
                  {rakeBack.monthly.amount ? (<div>Current bonus: <span className={classes.rbprice}>${ parseFloat(rakeBack.monthly.amount).toFixed(2) }</span></div>) : <div></div>}
                  <div onClick={() => { claimBonus('monthly') }}>{!isNaN(parseInt(rakeBack.monthly.amount)) ? 'Claim Bonus' : rakeBack.monthly.ftime}</div>
                </Box>
              </Grid>
              {/* RAKEBACK SYSTEM */}

              <div className={classes.rb}>Statistics</div>
