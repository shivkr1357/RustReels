...
const Affiliates = ({ isAuthenticated, isLoading, user, changeWallet }) => {
  ...
  const [affiliateLink, setAffiliateLink] = useState("LOADING...");
  ...
  const saveAffiliateCode = async () => {
    ...
    try {
      ...
      setAffiliateLink("https://silorust.com/a/" + response.newAffiliateCode);
              ...
              <Box className={classes.userWrap}>
                ...
                <Box className={classes.userWrap2}>
                  ...
                  <TextField
                    ...
                    disabled={loading}
                    value={loading ? "LOADING..." : affiliateLink}
                          ...
                          <TableRow key={user._id}>
                            <TableCell>
                              <Box display="flex" alignItems="center">
                                <Avatar src={user.user.avatar} variant="rounded" />
                                <Box ml="1rem" className={classes.desktop}>
                                  {user.user?.username || 'Deleted'}
                                </Box>
                              </Box>
                            </TableCell>
                            <TableCell>${(user.totalBet || 0).toFixed(2)}</TableCell>
                            <TableCell>${(user.totalDeposited || 0).toFixed(2)}</TableCell>
                            <TableCell>${(user.totalEarned || 0).toFixed(2)}</TableCell>
