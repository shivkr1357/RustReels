...
const SkinsBackDeposit = ({ open, handleClose, changeWallet, withItems, user }) => {
  ...
  const handleWithdrawItems = async () => {
    ...
    let resp = await tryToWithdrawItemsDOTA2(selectedItems.map(item => item.name), tradeUrl);

    const Success = () => {
      changeWallet({ amount: -Math.abs(totalAmount) });
      addToast(resp.msg, { appearance: "success", });
    };
