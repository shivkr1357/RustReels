...
const USDC = ({ user, changeWallet }) => {
  ...
  const onClick = async () => {
    setWithdrawing(true);
    try {
      const response = await withdrawCrypto("USDC", address, parseFloat(amount));
      ...
      if (response.state === 1) {
        addToast(
          `Successfully withdrawed ${response.siteValue.toFixed(
            ...
            8
          )} USDC! Your withdrawal should manual confirm within a few minutes!`,
          { appearance: "success" }
        );
      } else {
        addToast(
          `Successfully withdrawed ${response.siteValue.toFixed(
            2
          )} credits for USDC! Your withdrawal should manual confirm within a few minutes!`,
          { appearance: "success" }
        );
      }
    } catch (error) {
    ...
    <Box className={classes.root}>
      <Fragment>
        <Box className={classes.cryptocolor}>
          Withdraw your desired amount of USD Coin (USDC).
          <br /><br />
...
export default connect(mapStateToProps, { changeWallet })(USDC);
