...
const Solana = ({ user, changeWallet }) => {
  // Declare State
  ...
  const onClick = async () => {
    setWithdrawing(true);
    try {
      const response = await withdrawCrypto("SOL", address, parseFloat(amount));
      ...
      if (response.state === 1) {
        addToast(
          `Successfully withdrawed ${response.siteValue.toFixed(
            ...
            8
          )} Solana! Your withdrawal should manual confirm within a few minutes!`,
          { appearance: "success" }
        );
      } else {
        addToast(
          `Successfully withdrawed ${response.siteValue.toFixed(
            2
          )} credits for Solana! Your withdrawal should manual confirm within a few minutes!`,
          { appearance: "success" }
        );
      }
    } catch (error) {
    ...
    <Box className={classes.root}>
      <Fragment>
        <Box className={classes.cryptocolor}>
          Withdraw your desired amount of Solana (SOL).
          <br /><br />
...
export default connect(mapStateToProps, { changeWallet })(Solana);
